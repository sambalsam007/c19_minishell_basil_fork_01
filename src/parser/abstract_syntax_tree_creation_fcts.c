...
/*   Updated: 2025/01/07 13:16:11 by bclaeys          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	fill_all_values(t_token_node **curr_token_node,
							...
							t_var_data *var_data)
{
	...
	while ((*curr_token_node) && (*curr_token_node)->type != PIPE)
	{
		...
		{
			tmp_redir_node = add_redirect(curr_token_node, tmp_redir_node,
		...
		else if ((*curr_token_node)->type == EXEC)
